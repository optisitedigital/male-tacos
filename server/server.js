...
const GITHUB_REPO = process.env.GITHUB_REPO || 'optisitedigital/male-tacos';
...
async function getProductsFile() {
  if (!GITHUB_TOKEN || !GITHUB_REPO) {
    throw new Error('GITHUB_TOKEN ou GITHUB_REPO non configuré.');
  }

  const url = `https://api.github.com/repos/${GITHUB_REPO}/contents/${PRODUCTS_PATH}?ref=${encodeURIComponent(GITHUB_BRANCH)}`;
  ...
  if (!response.ok) {
    throw new Error(
      data.message || `Impossible de lire products.json (${response.status}).`
    );
  }

  if (!data.content) {
    throw new Error('GitHub n’a pas retourné le contenu de products.json.');
  }
  ...
  try {
    ...
    throw new Error('products.json contient un JSON invalide.');
  }
...
async function saveProductsFile(newContent, sha, message) {
  ...
  const body = {
    message: message || 'Mise à jour Malé Tacos',

    content: Buffer.from(JSON.stringify(newContent, null, 2), 'utf8').toString(
      'base64'
    ),
  ...
  if (!response.ok) {
    throw new Error(data.message || `Erreur GitHub (${response.status}).`);
  }
...
function validateProductsStructure(content) {
  if (!content || typeof content !== 'object' || Array.isArray(content)) {
    throw new Error('Structure de products.json invalide.');
  }
...
function createImageFileName(name) {
  const clean = sanitizeText(name, 'produit')
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
    .replace(/[^a-z0-9]+/g, '-')
...
async function uploadImageToGitHub(imageData, productName) {
  const parsed = validateImageData(imageData);
  ...
  const fileName = createImageFileName(productName);

  const imagePath = fileName;

  const url = `https://api.github.com/repos/${GITHUB_REPO}/contents/${encodeURIComponent(imagePath)}`;
  ...
  if (!response.ok) {
    throw new Error(
      data.message || `Impossible d'envoyer l'image (${response.status}).`
    );
  }
...
/*
|--------------------------------------------------------------------------
| GITHUB : NETTOYAGE DES IMAGES
|--------------------------------------------------------------------------
*/

/*
 * Seules les photos ajoutées par l'admin
 * (format : nom-1790251898655.jpg) peuvent être supprimées.
 * Les photos de base (burger.jpg, logo-male.png, etc.)
 * sont protégées.
 */

function isAdminImage(fileName) {
  return (
    typeof fileName === 'string' && /^[a-z0-9-]+-\d{13}\.jpg$/.test(fileName)
  );
}

function imageStillUsed(fileName, content) {
  const all = [...content.eleve, ...content.exterieur];

  return all.some(product => product.image === fileName);
}

async function deleteImageFromGitHub(fileName) {
  const base = `https://api.github.com/repos/${GITHUB_REPO}/contents/${encodeURIComponent(fileName)}`;

  const info = await fetch(`${base}?ref=${encodeURIComponent(GITHUB_BRANCH)}`, {
    headers: githubHeaders()
  });
  ...
  if (!info.ok || !data.sha) {
    throw new Error(
      data.message || `Lecture de l'image impossible (${info.status}).`
    );
  }
  ...
  const del = await fetch(base, {
    ...
    body: JSON.stringify({
      message: `Suppression image produit: ${fileName}`,
      sha: data.sha,
      branch: GITHUB_BRANCH
    })
  });
  ...
  if (!del.ok) {
    ...
    throw new Error(
      d.message || `Suppression de l'image impossible (${del.status}).`
    );
  }
}

async function cleanupImage(fileName, content) {
  try {
    if (!isAdminImage(fileName)) {
      return;
    }

    if (imageStillUsed(fileName, content)) {
      return;
    }
     ...
     * on note l'erreur sans bloquer.
     */

    console.error('Nettoyage image impossible:', error.message);
  }
}
...
app.post('/api/admin/products/:category', checkAdmin, async (req, res) => {
  try {
    ...
    let image = sanitizeText(body.image);

    if (body.imageData) {
      image = await uploadImageToGitHub(body.imageData, name);
    }
...
app.put('/api/admin/products/:category/:id', checkAdmin, async (req, res) => {
  try {
    ...
    const index = content[category].findIndex(product => product.id === id);
...
app.delete(
  ...
  async (req, res) => {
    try {
      ...
      const product = content[category].find(p => p.id === id);
      ...
      /*
       * Supprime aussi la photo du produit sur GitHub
       * (uniquement si elle a été ajoutée par l'admin
       * et si aucun autre produit ne l'utilise).
       */

      await cleanupImage(product.image, content);
...
app.listen(PORT, () => {
  console.log(`Malé Tacos server prêt sur le port ${PORT}`);

  console.log(`Repository GitHub: ${GITHUB_REPO}`);

  console.log(`Branche: ${GITHUB_BRANCH}`);
